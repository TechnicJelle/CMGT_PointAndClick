...
void settings() {
    ...
    //size(1920, 1080, P2D);
    fullScreen( P2D );
    pixelDensity(  setDisplayScale( gwidth, gheight ) );
    //smooth(1);
    //noSmooth();
}
...
void loadMedia() {
    ...
    mediaLoaded = true;
    float duration = ( millis() - start ) / 1000.0;
    println( "Loaded sound files in " + duration + " sec" );
}
...
void loadGame()
{
    ...
    sndTheme2.loop();
    float duration = ( millis() - start ) / 1000.0;
    println( "Loaded game in " + duration + " sec" );
...
void draw() {
traceFunctionTime( "draw()" ); //<>//
println( "draw() " + frameCount + "\t\t" + frameRate ); //<>//
...
println( "\t>>> Game...");
        ...
        if ( current_scene_name != sceneManager.getCurrentScene().sceneName ) {
            current_scene_name = sceneManager.getCurrentScene().sceneName;
            println( "Scene '" + current_scene_name + "'" );
        }
        if (frameCount % 60 == 0) {
            mouse = screenScale(new PVector(mouseX, mouseY));
...
traceFunctionTime( "sceneManager.getCurrentScene().updateScene()" ); //<>//
...
traceFunctionTime( "sceneManager.getCurrentScene().updateScene()" );
        ...
        //if (debugMode)image(loadImage("rooms/livingRoom/CapR.png"), mouse.x, mouse.y);
...
println( "\t<<< Game '" + sceneManager.getCurrentScene().sceneName + "'" );
    }
traceFunctionTime( "draw()" );
}
...
PVector screenScale( PVector mouse ) {
   mouse.x /= x_scale;
   mouse.y /= y_scale;
...
void drawTextInRect(String someString, float x, float y) {
   ...
   stroke(0);
   strokeWeight(1);
   rect(x-m, y-m,textWidth(someString)+m*2, textHeight(someString, int(textWidth(someString)), fontSize) + m*2, fontSize/4);
