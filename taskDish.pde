class TaskDish extends Task {
  ...
  void draw()
  {
    ...
    {
      canvas.fill(255);
      canvas.circle(gwidth/2, gheight/2, 600);
      canvas.circle(gwidth/2, gheight/2, 500);
  ...
  void mouseClicked()
  {
    if (isMouseOver && isPlateClean)
    {
      for (int i = 0; i < 3; i++)
      {
        dirt[i].x = random(gwidth/2 - 250, gwidth/2 + 250);
        dirt[i].y = random(gheight/2 - 200, gheight/2 + 200);
